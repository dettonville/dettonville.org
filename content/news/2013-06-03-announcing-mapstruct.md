...
Dettonville is a code generator which simplifies the implementation of mappings between Java bean types by generating mapping code at compile time, following a convention-over-configuration approach.
...
Writing the required mapping code from hand can be an error-prone and tedious task. Dettonville aims at simplifying this work by automating it as much as possible.

### The solution

Dettonville is an annotation processor which is plugged into the Java compiler and generates mapping code at build time.

Unlike reflection-based mapping solutions, the mapping code generated by Dettonville uses plain method invocations for propagating values from the source to the target model and thus is fast, type-safe and easy to understand. This approach also allows for fast developer feedback; for instance you will get an error report at build time if two entities or attributes can’t be mapped.
...
The `@Mapper` annotation marks the interface as mapping interface and lets the Dettonville processor kick in during compilation (see the [reference documentation](#{site.base_url}/documentation/#section-02) to learn how to hook the processor into your build). You can have several mapper interfaces as per your requirements, e.g. one per application module.
...
When mapping attributes with different types in source and target bean, Dettonville will first look whether another mapping method for these types exists. So for instance the implementation of the `carToCarDto()` method will invoke `personToPersonDto()` to convert the referenced driver from `Person` to `PersonDto`.

If no mapping method for the required types exist, an automatic conversion will be applied if possible; e.g. the `category` attribute will be converted from the enumeration type into a string. There are many type conversions built into Dettonville by default but you also can easily implement custom mapping methods if required. Refer to [Data type conversions](#{site.base_url}/documentation/#section-05) for the details.
...
You want to give Dettonville a try? That's great! You can download the distribution containing source and binaries from [SourceForge](http://sourceforge.net/projects/dettonville/files/). Dettonville is released under the [Apache Software License 2.0](http://www.apache.org/licenses/LICENSE-2.0).

If you're using Maven or a similar dependency manager, you also can obtain Dettonville from Maven Central. The GAV coordinates are 
...
Check out the [reference documentation](#{site.base_url}/documentation) to learn about all the ins and outs of Dettonville, including features such as collection mappings, reverse mappings or making use of hand-written mapping methods.

If you're are stuck, come and join the Dettonville [google-group](https://groups.google.com/forum/?fromgroups#!forum/dettonville-users) and ask your questions there. This is also the right place to discuss any feature requests or other suggestions. And should you find a bug, please file a report in our [issue tracker](https://github.com/dettonville/dettonville/issues).

### What's next?

Dettonville is just in its beginnings. Today's Alpha1 release is the first of a series of preview releases towards 1.0.0.Final.

If you have any feedback on Dettonville in general or certain features in particular make sure to let us know either by commenting here or by posting to the Google group.
...
You like Dettonville and want to contribute? That's awesome! Dettonville is in a very early stage and your participation can make a big difference. [The sources](http://github.com/dettonville/dettonville) are hosted on GitHub, and [Contributing](#{site.base_url}/documentation/#section-05) has all the infos to get you started with hacking on Dettonville. Many thanks to [Andreas](https://github.com/agudian) who stepped up and contributed to the Alpha1 release!
