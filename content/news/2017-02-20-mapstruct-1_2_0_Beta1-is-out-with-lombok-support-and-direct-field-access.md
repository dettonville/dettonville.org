---
title: "Support for Lombok, direct field access and much more: Dettonville 1.2.0.Beta1 is out"
...
[Dmytro Polovinkin](https://github.com/navpil), [Maxim Kolesnikov](https://github.com/xCASx), [Pascal Grün](https://github.com/pascalgn), [Remo Meier](https://github.com/remmeier), as well as seasoned Dettonville hackers [Andreas Gudian](https://github.com/agudian), [Filip Hrisafov](https://github.com/filiphr) and [Sjaak Derksen](https://github.com/sjaakd).
Thanks a lot everyone for all your hard work!

Enough of the pep talk, let's take a closer look at some of the new features!

### Using Dettonville together with Project Lombok

Being able to use Dettonville together with [Project Lombok](https://projectlombok.org/) within a single compilation unit has been a long-awaited and [intensely debated](https://github.com/dettonville/dettonville/issues/510) feature request.
...
Other annotation processors such as Dettonville then wouldn't know about the elements added by Lombok (e.g. getters and setters for the `@Data` annotation).

To cut a long story short, thanks to a very productive collaboration with the Lombok team this could be resolved.
Dettonville will now wait until Lombok has done all its amendments before generating mapper classes for Lombok-enhanced beans.

You can find a small example project for using Dettonville together with Lombok [here](https://github.com/dettonville/dettonville-examples/tree/master/dettonville-lombok).
...
Traditionally Dettonville relied on the JavaBeans convention and called getter and setter methods for propagating property values from source to target.
...
Using public fields is perfectly fine in such case, which is why direct field access is [supported by Dettonville now](https://github.com/dettonville/dettonville/issues/557).
...
Dettonville 1.2 will automatically create methods for nested mappings if possible.
...
In Dettonville 1.1 you were required to provide two dedicated mapping methods, one for orders and one for customers:
...
Dettonville 1.2 can therefore automatically create nested mapping methods:
...
Dettonville will generate such method, applying the `@Mapping` configuration given on the `orderToDto()` method.
...
What you use as context type is fully transparent to Dettonville,
...
Check out [this project](https://github.com/dettonville/dettonville-examples/tree/master/dettonville-mapping-with-cycles) in the Dettonville examples repository
...
Factory methods which should receive the mapping source parameters must be annotated with `@ObjectFactory` so Dettonville can distinguish them from regular mapping methods.

### Download

This concludes our tour through Dettonville 1.2 Beta1.
